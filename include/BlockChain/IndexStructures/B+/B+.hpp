  ...
  private:
    ...
    explicit Node(int _grado)
        : grado(_grado), keys(new value_type[_grado - 1]),
          children(new Node *[_grado]), next(nullptr), leaf(true) {}
    ~Node() {
      delete keys;
    ...
    Node(const Node &other)
        : keys(nullptr), children(nullptr), next(nullptr), count(other.count),
    ...
    void splitChildren(int index) {
      ...
      if (origin->leaf) {
        ...
        while (index_parent_insert >= 0 &&
               COMPARE(*keys[index_parent_insert], *mid_value)) {
        ...
        while (insert_parent_index >= 0 &&
               COMPARE(*keys[insert_parent_index], *origin->keys[grado / 2])) {
          keys[insert_parent_index + 1] = keys[insert_parent_index];
          insert_parent_index--;
        }
        keys[++insert_parent_index] =
            origin->keys[grado / 2]; // insert mid_value
        count++;
        leaf = false;
