...
public:
  ...
  explicit B(int _grado) : grado(_grado) { root = nullptr; }
  ~B() { delete root; }

  B(B &&other) noexcept : root(other.root) { other.root = nullptr; }
    ...
    while (!salvacion.empty()) {
      ...
      while (inner_index < temp->count) {
        ...
        if (temp->children[salvacion.top().second + 1] != nullptr) {
          temp->children[salvacion.top().second + 1]->getElements_rangeSearch(
              result, maxValue);
        }
        inner_index++;
      }
...
private:
  ...
  private:
    ...
    Node(Node &&other) noexcept
        : keys(other.keys), children(other.children), count(other.count),
    ...
    void rangeSearch(const index_type &minValue, const index_type &maxValue,
                     ...
                     stack<pair<const Node *, int>> &salavacion) const {
      ...
      if (keys[index]->get<compare_t>() == minValue) {
        ...
        while (inner_index < count &&
               keys[inner_index]->get<compare_t>() != maxValue) {
          vec.push_back(keys[inner_index]);
          inner_index++;
        }
      }
      if (children[index] != nullptr &&
          keys[index]->get<compare_t>() != minValue) {
    ...
    void getElements_rangeSearch(vector<value_type> &vec,
    ...
    [[nodiscard]] bool empty() const { return (this == nullptr); }
    ...
    void splitChildren(int index) {
      ...
      while (insert_parent_index >= 0 && COMPARE(*(keys[insert_parent_index]),
                                                 *(origin->keys[grado / 2]))) {
        keys[insert_parent_index + 1] = keys[insert_parent_index];
      }
      keys[++insert_parent_index] =
          origin->keys[grado / 2]; // inserting the middle value
      count++;
      leaf = false;
